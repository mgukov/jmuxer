...
export class Mp4 {
    ...
    private static dops(track: Track) {
        const audiosamplerate = track.audiosamplerate ?? 24000;
        const channelCount = track.channelCount ?? 1;

        return Box.make(Boxes.dOps, new Uint8Array([
            0x00, // version
            channelCount, // OutputChannelCount
            0x01, 0x38, // PreSkip
            0x00, 0x00, // InputSampleRate
            (audiosamplerate >> 8) & 0xFF,
            audiosamplerate & 0xff, //
    ...
    private static opus(track: Track) {
        const audiosamplerate = track.audiosamplerate ?? 24000;
        const channelCount = track.channelCount ?? 1;

        return Box.make(Boxes.Opus, new Uint8Array([
                0x00, 0x00, 0x00, // reserved
                0x00, 0x00, 0x00, // reserved
                0x00, 0x01, // data_reference_index
                0x00, 0x00, 0x00, 0x00,
    ...
    private static stsd(track: Track) {
        ...
        if (track.type === TrackType.Audio) {
            if (track.codec === 'opus') {
                codecBox = Mp4.opus(track);
            } else {
                codecBox = Mp4.mp4a(track);
            }
        } else {
            codecBox = Mp4.avc1(track);
        }
